			...
			_.forEach(['prerequestScript', 'assertions', 'testScript'], function (property) {
				_.forEach(execution[property], function (testExecution) {
					...
					if (property === 'assertions') {
						// Name
						testcase.att('name', `${className.join(SEPARATOR)}: ${testExecution.assertion}`);
						...
						var p
						p = property.att('name', 'test_summary');
						p = property.att('value', `${className.join(SEPARATOR)}: ${testExecution.assertion}`);
						testcase.att('properties', p)
